...
const baseService = new CompaniesService();

class CompaniesControllers extends BaseController {
  constructor() {
    super("a");
  }
  ...
  async list(req, res) {
    ...
    if (!req.body.where) { //user_id filter
      req.body.where = {};
    }
    req.body.where.user_id = req.user.id;
    return await baseService
      .list(req.body)
      .then((response) => {
        // console.log("res :>> ", response);
...
module.exports = new CompaniesControllers();
